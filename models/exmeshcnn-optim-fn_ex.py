...
class ExMeshCNN(nn.Module):
    ...
    def __init__(self, tag_data, params, gradcam = False):
        ...
        for tag, values in tag_data.items():
            ...
            fn_layer_count = params[f"{tag}__fn_layer_num"]

            act_fn_list = nn.ModuleList()

            for layer in range(fn_layer_count):
                fn_out_channel = params[f"{tag}__fn{layer}_out_channel"]
...
def get_model(tag_data, trial = None, params = None, gradcam = False):
    ...
    if trial is not None:
        ...
        for tag, _ in tag_data.items():
            ...
            # Set a number of layers
            params[f"{tag}__fn_layer_num"] = trial.suggest_int(f"{tag}__fn_layer_num", 1, 5)

            # And how dense each one is.
            for layer in range(params[f"{tag}__fn_layer_num"]):
                params[f"{tag}__fn{layer}_out_channel"]  = trial.suggest_categorical(f"{tag}__fn{layer}_out_channel", [8, 16, 32, 64, 128, 256])

...
BLEND_CMD = ["/home/valentino/Apps/blender-4.0.2-linux-x64/blender",
            "--background",
            "--python",
            "/media/valentino/Irithyll1/repos/datcom-tfm/scripts/meshes/cut_with_bbox__blender.py",
            "--"]

def blender_call(mesh, out, perc):
    if os.path.exists(os.path.join(out, os.path.split(mesh)[1])):
        print("Mesh already exists.")
    else:
        subprocess.run(BLEND_CMD + [mesh, out, str(perc)], check=False)
...
parser.add_argument("--cut-perc", type=float, default=0.7)
...
if args.cpus > 1:
    args_pool=[(m, args.out, args.cut_perc) for m in meshes]
    ...
    for i, m in enumerate(meshes):
        ...
        subprocess.run(BLEND_CMD + [m, args.out, str(args.cut_perc)], check=False)

...
BLEND_CMD = ["/home/valentino/Apps/blender-4.0.2-linux-x64/blender",
            "--background",
            "--python",
            "/media/valentino/Irithyll1/repos/datcom-tfm/scripts/meshes/decimation/collapse_edges_blender.py",
            "--"]

def blender_call(mesh, out, triangles):
    if os.path.exists(os.path.join(out, os.path.split(mesh)[1])):
        print("Mesh already exists.")
    else:
        subprocess.run(BLEND_CMD + [mesh, out, str(triangles)], check=False)
...
parser.add_argument("--cpus", default=4, type=int, help="Number of processes to use, default is 3.")
...
if args.cpus > 1:
    args_pool=[(m, args.out, args.trig_n) for m in meshes]
    ...
    for i, m in enumerate(meshes):
        ...
        subprocess.run(BLEND_CMD + [m, args.out, str(args.trig_n)], check=False)

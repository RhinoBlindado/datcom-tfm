...
BLEND_CMD = ["/home/valentino/Apps/blender-4.0.2-linux-x64/blender",
            "--background",
            "--python",
            "/media/valentino/Irithyll1/repos/datcom-tfm/scripts/meshes/mirror_mesh__blender.py",
            "--"]

def blender_call(mesh, out):
    mesh_name = os.path.split(mesh)[1].split(".")[0]
    mesh_name += "m.obj"
    if os.path.exists(os.path.join(out, mesh_name)):
